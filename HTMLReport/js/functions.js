...
var meta = { // {name, logo; logo if not given, then name is shown}
  os: {
    ...
    ios: { name: 'iOS', logo: 'https://upload.wikimedia.org/wikipedia/commons/f/fa/Apple_logo_black.svg' } 
  },
  browser: {
...
function addLog(ts, resId, tcId, tcName, os, browser, threadid, deviceName, eventType, detail,skips) {
  ...
  var deviceNameId = deviceName.toLowerCase();
...
`
  ...
  function buildRunDetails(row) {
	//var rw=row["results"]
	//var sumth =rw[threadid]
	 //var xyz = sumth[browser] 
	//for (var th in xyz){  
    for (os in meta.os) {
  ...
  function buildBr(info, report){
	if (info.logo){
		...
		if(report.result == "PASSED"){
			return `<a href="${report.startDetail}" style="text-decoration: none">${info.deviceName}<span class='successIndicator'>&#10004;</span></a>`
		}else if(report.result == "DONE"){
			return `<a href="${report.startDetail}" style="text-decoration: none">${info.deviceName}<span class='successIndicator'>&#10004;</span></a>`
		}else{
			return `<a href="${report.startDetail}" style="text-decoration: none">${info.deviceName}<span class='failureIndicator'>&#x2718;</span></a>`
		}
	}
    return info.name;
